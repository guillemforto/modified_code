...
"HTcondbiasest" <- function (data,
  ...
  if (missing(pkey)) {
    warning("Warning: no column has been specified as a primary key, one was added automatically (id)")
    id <- c(1:nrow(data))
  } else {
...
"strata_HTcondbiasest" <- function(data, strataname=NULL, varname=NULL, gnh,
                                   method=c("si","poisson","rejective"), pii, remerge=T) {
  if (missing(gnh))
  ...
  if (remerge==T){
    ...
    result = bc
    colnames(result) = paste0("condbias", colnames(bc))
  }
  result
}
